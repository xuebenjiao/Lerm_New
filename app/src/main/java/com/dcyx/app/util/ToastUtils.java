...
public class ToastUtils {
     ...
     *  {@code true}: 弹出新吐司<br>{@code false}: 只修改文本内容</p>
     *  如果为{@code false}的话可用来做显示任意时长的吐司</p>
     */
    ...
    private static void showToast(@StringRes int resId, int duration) {
        showToast(CustomApplication.getInstance().context.getResources().getText(resId).toString(), duration);
    }
    ...
    private static void showToast(@StringRes int resId, int duration, Object... args) {
        showToast(String.format(CustomApplication.getInstance().context.getResources().getString(resId), args), duration);
    }
    ...
    private static void showToast(CharSequence text, int duration) {
        ...
        if (sToast == null) {
            sToast = Toast.makeText(CustomApplication.getInstance().context, text, duration);
        } else {
            sToast.setText(text);
