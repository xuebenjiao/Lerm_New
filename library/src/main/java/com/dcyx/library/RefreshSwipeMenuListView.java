...
public class RefreshSwipeMenuListView extends ListView implements OnScrollListener {
    //需要添加左滑删除项的下标的字符串拼接
    public String needAddSwipeIndexStr;

    /**
     * 是否有多个ItemViewDelegate标识  默认是false
     */

    public boolean isHaveMutiItemTypeFlag = false;
    /**
     * 整个加载更多控件的背景颜色资源id
     */
    ...
    public static final int ALL_BOTH = 3;//禁止上拉和下拉
    ...
    public void setAdapter(final ListAdapter adapter) {
        ...
        super.setAdapter(new SwipeMenuAdapter(getContext(), adapter,isHaveMutiItemTypeFlag) {
            @Override
            ...
            case MotionEvent.ACTION_DOWN:   //手势按下事件、获取坐标、设置上次下拉时间
                ...
                if (isSwipe) {
                    ...
                    if (mTouchView != null && mTouchView.isOpen()) {
                        mTouchView.smoothCloseMenu();
                        mTouchView = null;
                        return super.onTouchEvent(ev);//如果将该代码发在最后，可以实现即关闭上次又打开本次
                    }
...
//
    ...
    public void setPullRefreshEnable(boolean enable) {
        ...
        if (!mEnablePullRefresh) { // disable, hide the content
            mHeaderViewContent.setVisibility(View.INVISIBLE);//View.INVISIBLE
        } else {
...
//            if (mEnablePullRefresh && !mPullRefreshing) {  //xbj
            if(!mPullRefreshing){
    ...
    private boolean canLoadMore() {
        return isBottom() &&mEnablePullLoad&& !mPullLoading && isPullingUp();
    }
    ...
    /**
     * 设置ListView是否有多个ItemType
     * @param haveMutiItemTypeFlag  默认false
     */
    public void setHaveMutiItemTypeFlag(boolean haveMutiItemTypeFlag) {
        isHaveMutiItemTypeFlag = haveMutiItemTypeFlag;
    }
